...
if ( ! class_exists( 'BaseACFLinkHelper' ) ) {
	 ...
	 * Code example:
	 * if ( BaseACFLinkHelper::isLink( $link ) ) BaseACFLinkHelper::theLink( $link, ['btn', 'btn-primary'] );
	 */
	class BaseACFLinkHelper {
		 ...
		 * @return string
		 */
		public static function getLink( $link, $classes = array() ) {
			$_url = $link['url'];
			$_title = $link['title'];
		 ...
		 * @param array $classes
		 */
		public static function theLink( $link, $classes = array() ) {
			echo self::getLink( $link, $classes );
		}
